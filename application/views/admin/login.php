    ...
    <div class="loginColumns animated fadeInDown">
        <div class="row">
            <div class="col-md-6 col-md-offset-3" style="padding-top: 45px">
                <div class="ibox-content" style="background: #3f48cc;">
                    <img src="<?php echo base_url();?>assets/images/logo-icon-150x150.png" class="img-responsive" style="margin:0 auto;">						<form name="loginSubmit" id="loginSubmit" method="POST" >
                        <hr>
                        <h2 class="font-bold text-center" style="color: #fff"><b>LOGIN</b></h2>
                        ...
                        <div class="form-group">
							...
							<span id="error_u_password" class="error_mesge_c"></span>                        </div>
						<br>                        <input onClick="validateLoginForm();" type="button" name="login" class="btn btn-primary block full-width m-b" value="Login">                        <a href="#">                            <br>                            <small style="color: #fff">Forgot password?</small>                        </a>
                    </form>
                    <p class="m-t">  
                    </p>
                </div>
            ...
            <hr/>
            <div class="col-md-6">
                Copyright <a href="javascript:void(0);">Vclean4U</a> &copy; <?=date('Y');?>-<?=date('Y', strtotime('+1 years'));?></a>
            </div>
            <div class="col-md-6 text-right">
               <small> Made With <i class="fa fa-heart" style="color: red"></i> By <a href="javascript:void(0);">Vclean4U</a></small>
            </div>
        </div>
    </div>
    ...
    <script>
        $(document).ready(function() {
			setTimeout(function() {
				...
				<?php if($this->session->flashdata('success_message')){?>					toastr.success(<?= $this->session->flashdata('success_message') ?>);				<?php } ?>				<?php if($this->session->flashdata('error_message')){?>					toastr.error(<?= $this->session->flashdata('error_message') ?>);				<?php } ?>				<?php if($this->session->flashdata('warning_message')){?>					toastr.warning(<?= $this->session->flashdata('warning_message') ?>);				<?php } ?>				<?= ((validation_errors()) && (null !== validation_errors())) ? "toastr.warning('".removeNewLine(removepTag(validation_errors()))."');":""?>			}, 1300);		});
        $(function() {
